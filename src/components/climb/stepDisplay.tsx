...
export default function StepDisplay({
	...
	const wallId =
		typeof window !== "undefined" ? localStorage.getItem("wallId") : null;
	...
	useEffect(() => {
		...
		const generate = async () => {
			try {
				...
				const difficulty = "easy"; // can be dynamic
				...
				// const pathDataRaw = localStorage.getItem("aiGeneratedPath");
				// if (!pathDataRaw) {
				//   throw new Error("Missing stored path data.");
				// }
				// const pathData = JSON.parse(pathDataRaw);
				// if (!pathData.route?.length) {
				// 	throw new Error(pathData.error || "Upss..! Please click the button and try again.");
				// }
				...
				const overlayRes = await fetch("/api/overlay-path", {
					...
					body: JSON.stringify({
						imageUrl,
						route: pathData.route?.length ? pathData.route : pathData.allHolds,
						userParams,
						difficulty,
						fallback: !pathData.route?.length, // Add this flag
					}),
				});
